...
pick_port() {
  ...
  for _ in $(seq 1 200); do
    ...
    if ! ss -ltn | rg -q ":${candidate}\\b"; then
      echo "$candidate"
      return 0
    fi
  done
...
error_rate_pct="$(awk -v e="$error_requests" -v t="$total_requests" 'BEGIN {if (t==0) print "0.00"; else printf "%.2f", (e*100.0)/t}')"
...
echo "SOAK_RESULT duration_sec=${SOAK_DURATION_SEC} requests=${total_requests} errors=${error_requests} error_rate_pct=${error_rate_pct} models_avg_ms=${avg_ms} models_p95_ms=${p95_ms} goroutine_delta=${goroutine_delta} open_fds_delta=${open_fds_delta} heap_delta_bytes=${heap_delta_bytes}"
echo "SOAK_LIMITS p95<=${MAX_MODELS_P95_MS} err_rate<=${MAX_ERROR_RATE_PCT}% goroutine_delta<=${MAX_GOROUTINE_DELTA} open_fds_delta<=${MAX_OPEN_FDS_DELTA} heap_delta<=${SOAK_MAX_HEAP_DELTA_BYTES}"
if [[ "$PPROF_ENABLED_BOOL" == "true" ]]; then
